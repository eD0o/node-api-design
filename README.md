...
A good database schema should be easy to understand, safe to change, and efficient to query.
...
Timestamps are useful for debugging, auditing, sorting, and understanding how data changes over time.

### Enforce Constraints in the Database

Important rules should also exist at the database level.
...
FOREIGN KEY  → keeps relationships valid
```

Frontend or middleware validation can be bypassed, but database constraints protect the data itself.

### Index Frequently Queried Columns

Indexes help the database find rows without scanning the entire table.
...
may benefit from an index on `user_id`:

```sql
CREATE INDEX idx_habits_user_id ON habits(user_id);
```

Typical candidates include frequently searched or joined columns such as:
...
Database query plans and performance tools can help identify which indexes are actually useful.

> Add indexes based on real query patterns instead of indexing everything.
...
A schema should be understandable without guessing what each field means.

### Plan for Schema Evolution

Database schemas will change as new features are added.
...
4. Copy old data into the new column
5. Remove the old column later
```

This reduces the risk of breaking code that still depends on the old schema.

### Normalize Appropriately

Normalization reduces duplicated data and keeps relationships organized.

However, **over-normalization** can make the application unnecessarily complex by requiring too many joins.

The goal is not maximum normalization. The goal is a schema that is consistent and practical for how the application queries data.

### Prefer Enums for Fixed Values

If a field can only contain a known set of values, an enum is usually better than unrestricted text.

For example, `frequency` could be limited to:
...
This makes the data more predictable for both the backend and frontend.

### Reuse Database Schemas for Validation

Schemas generated from Drizzle can be reused with validation middleware:
...
```ts
app.post("/users", validateBody(insertUserSchema), createUser);
```

This keeps API validation aligned with the database schema.

However, use a customized or partial schema when the user does not provide every field expected by the database.
...
may be generated by the application or database instead of coming from the request.
...
Missing Indexes
→ frequent queries may require table scans

Over-Normalization
→ too many unnecessary tables and joins

Missing Timestamps
→ harder to track when data was created or changed
